...
def get_metrics(score, labels, slidingWindow=100, pred=None, version='opt', thre=250):
    ...
    # VUS-ROC和VUS-PR：体积下面积指标，考虑了时间窗口因素
...
def get_metrics_pred(score, labels, pred, slidingWindow=100):
    ...
    PointF1 = grader.metric_PointF1(labels, score, preds=pred)      # 基于point
    PointF1PA = grader.metric_PointF1PA(labels, score, preds=pred)  # 基于point adjust
    EventF1PA = grader.metric_EventF1PA(labels, score, preds=pred)  # 基于event
    RF1 = grader.metric_RF1(labels, score, preds=pred)              # 基于范围
    Affiliation_F = grader.metric_Affiliation(labels, score, preds=pred)    # 基于关联: 预测异常和真实异常
    VUS_R, VUS_P, VUS_F = grader.metric_VUS_pred(labels, preds=pred, windowSize=slidingWindow)  # 体积指标
